...
//   try {
//     const txns = await Transaction.find({ user: req.userId }).sort({ date: -1, createdAt: -1 });
//     res.json(txns);
//   } catch (err) {
//     res.status(500).json({ message: err.message });
//   }
...
// // @desc Add a transaction for logged-in user
// router.post("/", auth, async (req, res) => {
//   try {
//     const { category, method, amount, currency, date, notes } = req.body;
...
// //     // Ownership check
// //     if (String(txn.user) !== String(req.userId)) {
...
//     // Attempt to delete the document that matches BOTH id and user
//     const deleted = await Transaction.findOneAndDelete({ _id: txnId, user: req.userId });

//     if (!deleted) {
//       // If nothing deleted, either it didn't exist or user doesn't own it.
//       // Return 404 to avoid leaking ownership info, or 403 if you prefer.
//       return res.status(404).json({ message: "Transaction not found or not authorized" });
//     }

//     return res.json({ message: "Transaction deleted", id: txnId });
//   } catch (err) {
//     console.error("Delete txn error:", err);
