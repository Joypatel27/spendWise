...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // notify other parts of the app (DataContext) about auth change
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-center" style={{ marginBottom: "8px", fontWeight: 800, fontSize: "28px" }}>
          Welcome Back!
        </h2>
        ...
        <Form onSubmit={handleLogin} noValidate>
          <Form.Group className="mb-3">
            ...
            <Form.Control
              ...
              onChange={(e) => setEmail(e.target.value)}
              isInvalid={showInvalid(email)}
              style={{ height: "46px", borderRadius: "8px", paddingLeft: "12px" }}
            />
          ...
          <Button
            ...
            className="w-100 d-flex align-items-center justify-content-center"
            disabled={loading}
            style={{ height: "48px", borderRadius: "8px", fontSize: "16px", fontWeight: 700, letterSpacing: 0.2 }}
          >
            {loading ? (
              <>
                <Spinner as="span" animation="border" size="sm" role="status" aria-hidden />
        ...
        <div style={{ textAlign: "center", marginTop: "18px", fontSize: "15px", color: "#374151" }}>
          <span>Don't have an account? </span>
          <Link to="/signup" style={{ marginLeft: 6, fontWeight: 600 }} onClick={() => setSubmitted(false)}>
            Register
          </Link>
        </div>

        <div style={{ textAlign: "center", marginTop: 12 }}>
          <Link to="/forgot-password" style={{ fontSize: 14, color: "#2563eb" }} onClick={() => setSubmitted(false)}>
            Forgot Password?
          </Link>
        </div>
      </Card>
